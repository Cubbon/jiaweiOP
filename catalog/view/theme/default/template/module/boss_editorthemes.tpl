...
<?php if (file_exists('catalog/view/theme/'.$this->config->get('config_template').'/stylesheet/boss_themes.css')) {
		echo '<link rel="stylesheet" type="text/css" href="catalog/view/theme/'.$this->config->get('config_template').'/stylesheet/boss_themes.css" media="screen" />';
	} else {
		echo '<link rel="stylesheet" type="text/css" href="catalog/view/theme/default/stylesheet/boss_themes.css" media="screen" />';
	}
?>
		...
		foreach ($objXML->children() as $child){
			foreach($child->children() as $childOFchild){
				if($childOFchild->getName()=='background'){
					foreach($childOFchild->children() as $childOF){ 
						if($childOF->name!=''){ ?>
							$("<?php echo $childOF->class ?>").css('background-color','#' + themeData['<?php echo $childOF->name ?>']);
					...
					foreach($childOFchild->children() as $childOF){ 
						if($childOF->name!=''){ ?>
							$("<?php echo $childOF->class ?>").css('color','#' + themeData['<?php echo $childOF->name ?>']);
							$("#<?php echo $childOF->name ?>").css('background', '#' + themeData['<?php echo $childOF->name ?>']);
							$("#<?php echo $childOF->name ?>").val(themeData['<?php echo $childOF->name ?>']);
						<?php }
					}
			...
			foreach ($objXML->children() as $child){
				foreach($child->children() as $childOFchild){
					if($childOFchild->getName()=='background'){
						foreach($childOFchild->children() as $childOF){ 
							if($childOF->name!=''){ ?>
								$("<?php echo $childOF->class ?>").css('background-color',"#<?php echo $b_Color_Data[''.$childOF->name.''] ?>");
						...
						foreach($childOFchild->children() as $childOF){ 
							if($childOF->name!=''){ ?>
								$("<?php echo $childOF->class ?>").css('color',"#<?php echo $b_Color_Data[''.$childOF->name.''] ?>");
								$("#<?php echo $childOF->name ?>").css('background', "#<?php echo $b_Color_Data[''.$childOF->name.''] ?>");
								$("#<?php echo $childOF->name ?>").val("<?php echo $b_Color_Data[''.$childOF->name.''] ?>");
							<?php }
						}
