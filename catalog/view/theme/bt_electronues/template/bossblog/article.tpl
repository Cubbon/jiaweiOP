    ...
    <div class="article-title">
        ...
        <p><?php echo $title;?></p>
			...
			<div id="productRelated">
				<h3 class="form-title"><span><?php echo $text_product_related ?></span></h3>
				<div class="box-product list_carousel responsive">
					...
					<?php foreach ($products as $product) {?><li> 
						...
						<?php } ?>
						<div class="cart"><a class="button button_add" onclick="boss_addToCart('<?php echo $product['product_id']; ?>');"><span><?php echo $button_cart; ?></span></a></div>
    					</div></li><?php } ?>
					</ul>
                    ...
                    <div id="0_comment_box" class="form-comment">
                    	  <?php if(!$login){?>
                    		  ...
                    		  <div class="field" id="email">
                    			  ...
                    			  <div class="input-box">
                    				  <input type="text" class="input-text required-entry validate-email" value="" title="Email" id="email" name="email_address">
                    			  </div>
                    		  </div>
                    	  <?php } else{?>
                                <input type="hidden" class="input-text required-entry" value="<?php echo $username; ?>" title="Name" id="name" name="username">
                                <input type="hidden" class="input-text required-entry validate-email" value="<?php echo $email; ?>" title="Email" id="email" name="email_address">
                          <?php } ?>
                    	  <div class="input-box">
                    		  <label class="required" for="comment"><?php echo $entry_comment; ?><em>*</em></label>
                    		  <textarea rows="2" cols="10" class="required-entry input-text" style="height:200px" title="Comment" id="comment" name="comment_content"></textarea>
                    	  </div>
...
$('#button-comment').bind('click', function() {
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'username=' + encodeURIComponent($('input[name=\'username\']').val()) + '&comment_content=' + encodeURIComponent($('textarea[name=\'comment_content\']').val()) + '&email=' + encodeURIComponent($('input[name=\'email_address\']').val()) + '&captcha=' + encodeURIComponent($('input[name=\'captcha\']').val()),
		beforeSend: function() {
