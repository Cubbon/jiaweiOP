<?php
class ModelModuleBossTagCloud extends Model {
	...
	private function generateTagCloud($tags, $resize = true, $min_font_size = 9, $max_font_size = 25, $font_weight) {
		...
		if ($resize == true) {
			...
			foreach ($tags as $key => $value) {
				...
				$cloud[] = '<a href="' . $this->url->link('product/search', 'filter_tag=' . $value) . '" style="text-decoration:none;font-size:' . $size . 'px;font-weight:' . $font_weight . ';" title="">' . $value . '</a> ';
			}
			
		} else {
		
			foreach ($tags as $key => $value) {
				$cloud[] = '<a href="' . $this->url->link('product/search', 'filter_tag=' . $value) . '" style="text-decoration:none;" title="">' . $value . '</a> ';
			}
		}
