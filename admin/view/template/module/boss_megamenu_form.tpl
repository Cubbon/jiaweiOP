...
<div>
	...
	<h4>Menu Icon</h4>
	<table class="form">
		<?php $icon = json_decode($menu['image'], true); ?>
		<tr>
			<td>Icon</td>         
			<td>
				<div class="image">
				  <img src="<?php echo $thumb; ?>" alt="" id="thumb" />
				  <input type="hidden" name="megamenu[image][icon]" value="<?php echo $icon['icon']; ?>" id="image"  />
				  <br />
				  <a onclick="image_upload('image', 'thumb');"><?php echo $text_browse; ?>
				  </a>&nbsp;&nbsp;|&nbsp;&nbsp;<a onclick="$('#thumb').attr('src', '<?php echo $no_image; ?>'); $('#image').attr('value', '');"><?php echo $text_clear; ?></a>
				</div>
			</td>
		</tr>
		<tr>
			<td>Icon (W x H)</td>
			<td>
				<input type="text" name="megamenu[image][icon_image_width]" value="<?php echo $icon['icon_image_width']; ?>" size="3" />
				<input type="text" name="megamenu[image][icon_image_height]" value="<?php echo $icon['icon_image_height']; ?>" size="3" />
			</td>
		</tr>	
	</table>
	<h4>Menu Type</h4>
					...
					<div class="scrollbox" id="product_list">
						...
						<?php if($ids[0]!='') { ?>
							<?php foreach ($ids as $product_id) { ?>
								<?php $divclass = ($divclass == 'even' ? 'odd' : 'even'); ?>
<div class="<?php echo $divclass; ?>" id="product-item-<?php echo $product_id; ?>"><?php echo $products_name[$product_id]; ?>
